@kwdef mutable struct ContextTracker{T}
    type::T # DataType{T}
    age_filter::AgeTracker=AgeTracker()
    updated_sources::Vector{String} = String[]
    new_sources::Vector{String} = String[]
end

function (node::ContextTracker)(result::RAGContext, args...)
    add_or_update_source!(node, result.chunk.sources, result.chunk.contexts)
    return format_context_node(node)
end

function add_or_update_source!(node::ContextTracker{T}, sources::Vector{String}, contexts::Vector{<:AbstractString}; verbose=true) where T
...
function format_context_node(node::ContextTracker{T}) where T
...
function format_files(node::ContextTracker{T}, sources::Vector{String}) where T
    formatted_files = ""
...
function cut_history!(node::ContextTracker, keep::Int)
