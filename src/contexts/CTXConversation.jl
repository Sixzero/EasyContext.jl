...
get_next_msg_contrcutor(conv::ConversationCTX) = conv.conv.messages[end].role === :user ? create_AI_message : create_user_message
...
update_last_user_message_meta(conv::ConversationCTX, meta) = update_last_user_message_meta(conv.conv, meta["input_tokens"], meta["output_tokens"], meta["cache_creation_input_tokens"], meta["cache_read_input_tokens"], meta["price"], meta["elapsed"]) 
update_last_user_message_meta(conv::ConversationCTX, itok::Int, otok::Int, cached::Int, cache_read::Int, price, elapsed::Float64) = update_last_user_message_meta(conv.conv, itok, otok, cached, cache_read, price, elapsed)
