...
Anthropic.ai_stream_safe(conv::ToSolve; model, system_msg=conv.system_message, max_tokens::Int=MAX_TOKEN, printout=true, cache=nothing) = ai_stream_safe(to_dict_nosys(conv); system_msg=system_msg.content, model, max_tokens, printout, cache)
...
anthropic_ask_safe(conversation::ToSolve; model, return_all=false, cache=nothing) = begin
    conv = [
