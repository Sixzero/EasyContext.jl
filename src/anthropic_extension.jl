...
ai_stream(conv::Session; model, system_msg=conv.system_message, max_tokens::Int=MAX_TOKEN, printout=true, cache=nothing, top_p=nothing, stop_sequences=[]) = ai_stream_safe(to_dict_nosys(conv); system_msg=system_msg.content, model, max_tokens, printout, cache, top_p, stop_sequences)
...
anthropic_ask_safe(conversation::Session; model, return_all=false, cache=nothing) = begin
    conv = [
