...
ai_stream(conv::ConversationX; model, system_msg=conv.system_message, max_tokens::Int=MAX_TOKEN, printout=true, cache=nothing, top_p=nothing) = ai_stream_safe(to_dict_nosys(conv); system_msg=system_msg.content, model, max_tokens, printout, cache, top_p)
...
anthropic_ask_safe(conversation::ConversationX; model, return_all=false, cache=nothing) = begin
    conv = [
