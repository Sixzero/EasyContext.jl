...
Anthropic.ai_stream_safe(conv::ConversationInfo; model, system_msg=conv.system_message, max_tokens::Int=MAX_TOKEN, printout=true, cache=nothing) = ai_stream_safe(to_dict_nosys(conv); system_msg=system_msg.content, model, max_tokens, printout, cache)
