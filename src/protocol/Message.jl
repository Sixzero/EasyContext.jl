...
create_AI_message(ai_message::AbstractString, meta::Dict, context::Dict{String,String}=Dict{String,String}()) = Message(timestamp=now(UTC), role=:assistant, content=ai_message, context=context, itok=meta["input_tokens"], otok=meta["output_tokens"], cached=meta["cache_creation_input_tokens"], cache_read=meta["cache_read_input_tokens"], price=meta["price"], elapsed=meta["elapsed"], stop_sequence=get(meta, "stop_sequence", ""))
create_AI_message(ai_message::AbstractString)             = Message(timestamp=now(UTC), role=:assistant, content=ai_message)
